...
#define R2FS_ITEST_LARGE_FILE_PATH "/large-boundary.bin"

typedef struct R2fsLargeFileBoundaryCase
{
    uint32_t block_index;
    char fill;
} R2fsLargeFileBoundaryCase;

static void r2fsLargeFileFill(
    char *buffer,
    size_t size,
    char fill
)
...
{
    static const R2fsLargeFileBoundaryCase cases[] = {
    ...
    TEST_ASSERT_EQUAL(
        0,
        r2fsRtemsMountFixtureCreateFile(R2FS_ITEST_LARGE_FILE_PATH, 0644)
    );
    TEST_ASSERT_EQUAL(
        0,
        r2fsRtemsMountFixtureOpen(
            R2FS_ITEST_LARGE_FILE_PATH,
            O_RDWR,
            0,
            &fd
        )
    ...
    for (i = 0; i < sizeof(cases) / sizeof(cases[0]); ++i) {
        ...
        r2fsLargeFileFill(expected, sizeof(expected), cases[i].fill);
        offset = (off_t)((uint64_t)cases[i].block_index * BLOCK_BUFFER_SIZE);

        TEST_ASSERT_EQUAL(
            offset,
            r2fsRtemsMountFixtureLseek(fd, offset, SEEK_SET)
        );
    ...
    TEST_ASSERT_EQUAL(
        0,
        r2fsRtemsMountFixtureStatPath(R2FS_ITEST_LARGE_FILE_PATH, &st)
    );
    expected_size =
        (off_t)((uint64_t)cases[
            (sizeof(cases) / sizeof(cases[0])) - 1u
        ].block_index * BLOCK_BUFFER_SIZE) + (off_t)sizeof(expected);
